...
/*
EXEC dbo.rptCust_Post_Event_LastTimeAtCenter_SSB  @TYPE = 'ET', -- varchar(2)
    @GROUPING = 'BWAY', -- varchar(25)
    @Season = 'TSC1415' -- varchar(15)
*/


CREATE PROCEDURE [dbo].[rptCust_Post_Event_LastTimeAtCenter_SSB]
    @TYPE AS VARCHAR(2)
...
--DROP TABLE #tagtemp
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - LastTimeAtCenterorderHHStart'
		...
		CREATE INDEX CustomerOrder ON #orderedHH(customer)

		        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - LastTimeAtCenterorderHHEND'
-------------------------------------------------------------------
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - LastTimeAtCenterlastpurchasedStart'
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - LastTimeAtCenterlastpurchasedEND'
