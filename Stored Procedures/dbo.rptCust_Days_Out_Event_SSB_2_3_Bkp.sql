...
/*
EXEC dbo.rptCust_Days_Out_Event_SSB_2_3 @Season = 'LVP1213', -- varchar(25)
...
CREATE PROCEDURE [dbo].[rptCust_Days_Out_Event_SSB_2_3_Bkp]
    @Season VARCHAR(500)
...
--Event Report for CUSTOM - Event Days Out Report
        ...
        INTO    #Temp2
        FROM    #Temp tp
                INNER JOIN vTK_Event evt ON evt.SeasonCode = tp.SeasonCode
        ...
        SELECT  tp.SeasonCode
              ...
              , tp.ValueType
              , tp.Value
			  , tp.SeasonCode COLLATE SQL_Latin1_General_CP1_CS_AS + tp.EventCode COLLATE SQL_Latin1_General_CP1_CS_AS AS 'SeasonEventGroup'
        FROM    #Temp2 tp
                INNER JOIN vTK_Event evt ON evt.SeasonCode = tp.SeasonCode
                                            AND evt.EventCode = tp.EventCode
                INNER JOIN vTK_EGroup eg ON eg.SeasonCode = tp.SeasonCode
                                            AND eg.EventGroupCode = evt.EventGroupCode
        WHERE   tp.SeasonCode COLLATE SQL_Latin1_General_CP1_CS_AS IN (SELECT Item FROM [dbo].[SplitSSB] (@Season,','))
                AND tp.EventTypeCode COLLATE SQL_Latin1_General_CP1_CS_AS IN (SELECT Item FROM [dbo].[SplitSSB] (@EventType,','))
                AND tp.EventCode COLLATE SQL_Latin1_General_CP1_CS_AS IN (SELECT Item FROM [dbo].[SplitSSB] (@Event,','))
				AND tp.FacilityCode COLLATE SQL_Latin1_General_CP1_CS_AS IN (SELECT Item FROM [dbo].[SplitSSB] (@Venue,','))
