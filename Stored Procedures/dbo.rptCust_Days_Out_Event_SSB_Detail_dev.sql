...
--EXEC [dbo].[rptCust_Days_Out_Event_SSB_Detail_dev] 'TSC1516','ET','BWAY OSD-04222016 P1','BWAY'
CREATE PROCEDURE [dbo].[rptCust_Days_Out_Event_SSB_Detail_dev]
    @Season VARCHAR(500)
  , @Type VARCHAR(25)
...
*/
        ...
        SET @MINDATE = ( SELECT MIN(trans.DATE)
                         ...
                         WHERE  ( ( @Type = 'EG'
                                  ...
                                  --   )
                                  OR ( @Type = 'ET'
                                       --AND ev.TAG IN (SELECT Item COLLATE SQL_Latin1_General_CP1_CS_AS FROM [dbo].[SplitSSB] (@Grouping,','))
                                     )
                                )
        ...
        SET @MAXDAYSOUT = ( SELECT  MAX(DATEDIFF(d, trans.DATE, event.DATE))
                            ...
                            WHERE   ( ( @Type = 'EG'
                                  ...
                                  --   )
                                      OR ( @Type = 'ET'
                                           AND event.TAG IN (SELECT Item COLLATE SQL_Latin1_General_CP1_CS_AS FROM [dbo].[SplitSSB] (@Grouping,','))
                                         )
                                    )
									AND trans.SEASON IN (SELECT Item FROM [dbo].[SplitSSB] (@Season,','))
        ...
        WHERE   dates.alldays <= @MAXDAYSOUT
                AND sales.SEASON IN (SELECT Item FROM [dbo].[SplitSSB] (@Season,','))
