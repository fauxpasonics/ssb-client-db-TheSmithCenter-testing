...
/*
EXEC dbo.rptCust_Post_Event_FrequnecyAllTime_SSB  @TYPE = 'EV', -- varchar(2)
    @GROUPING = 'R0430', -- varchar(25)
    @Season = 'TSC1415' -- varchar(15)
*/


CREATE PROCEDURE [dbo].[rptCust_Post_Event_FrequnecyAllTime_SSB]
    @TYPE AS VARCHAR(2)
    ...
    BEGIN
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - FrequencyAllTimeEnter' 
...
--DECLARE @Season AS VARCHAR(15) = 'TSC1718'
        ...
        INSERT  INTO #tscEvents
                ...
                SELECT  SEASON
                      , EVENT
                FROM    dbo.TK_EVENT e WITH ( NOLOCK )
...
------------------------------------------------------------------------------------------------
        ...
        INSERT  INTO #orderedHH
                ...
                INNER JOIN dbo.TK_EVENT tkEvent WITH ( NOLOCK ) 
					ON ( tkTransEvent.SEASON = tkEvent.SEASON
                    AND tkTransEvent.EVENT = tkEvent.EVENT)
				INNER JOIN dbo.TK_PRTYPE prtype WITH ( NOLOCK ) 
					ON ( prtype.SEASON = tkTransEvent.SEASON
					AND prtype.PRTYPE = tkTransEvent.E_PT
					)
				INNER JOIN #SeasonTemp st
					on tkTransEvent.SEASON = st.Item
...
-------------------------------------------------------------------
        ...
        INSERT  INTO #pastpurch
				...
				INNER JOIN dbo.TK_PRTYPE prtype WITH ( NOLOCK ) ON ( prtype.SEASON = tkTransEvent.SEASON
                    AND prtype.PRTYPE = tkTransEvent.E_PT
                    )
...
-------------------------------------------------------------------
        ...
        INSERT  INTO #purchases
                ...
                LEFT JOIN #tscEvents tscEvent 
					ON ( tscEvent.SEASON COLLATE SQL_Latin1_General_CP1_CS_AS = purch.season
...
--INTO [dbo].[TEMP_rptCust_Post_Event_FrequnecyAllTime_SSB]
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - FrequencyAllTimeExit' 

--SELECT * FROM [dbo].[TEMP_rptCust_Post_Event_FrequnecyAllTime_SSB]
