...
/*
EXEC dbo.rptCust_Post_Event_AvgOrderZipCodes_SSB  @TYPE = 'ET', -- varchar(2)
    @GROUPING = 'BWAY', -- varchar(25)
    @Season = 'TSC1415' -- varchar(15)
*/


CREATE PROCEDURE [dbo].[rptCust_Post_Event_AvgOrderZipCodes_SSB]
    @TYPE AS VARCHAR(2)
    ...
    BEGIN
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - AvgOrderZipCodesEnter' 
...
-------------------------------------------------------------------
        ...
        INSERT  INTO #orderedHH
                ...
                INNER JOIN dbo.TK_CUSTOMER tkCustomer WITH ( NOLOCK ) ON ( tkCustomer.CUSTOMER = trans.CUSTOMER )
...
-------------------------------------------------------------------
        ...
        SELECT TOP 5
              ...
              , ROUND(CAST(COUNT(oHH.customer) AS DECIMAL) / ( SELECT
                                                              COUNT(customer)
                                                              FROM
                                                              #orderedHH
                                                              ), 2) AS HH_Percent
	--	INTO [dbo].[TEMP_rptCust_Post_Event_AvgOrderZipCodes_SSB]
        ...
        GROUP BY zip.zipCode
              , sysZip.CSZ
        HAVING  COUNT(oHH.customer) > 7 --From Print out - Only show if total of households is greater than 7
        ...
        INSERT  [dbo].[TempVariableTrap]
                SELECT  @Season + '|' + @TYPE + '|' + @GROUPING + '|'
                        + '|'
                      , GETDATE()
                      , 'PostEvent - AvgOrderZipCodesExit' 
