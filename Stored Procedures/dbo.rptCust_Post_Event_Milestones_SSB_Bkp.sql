...
CREATE PROCEDURE [dbo].[rptCust_Post_Event_Milestones_SSB_Bkp]
    @TYPE AS VARCHAR(2)
...
--DECLARE @Season AS VARCHAR(15) = 'TSC1415'
        ...
        select distinct Customer into #hivalcust from
		(select Customer from dbo.TK_TRANS_ITEM_EVENT group by Customer having sum(E_OQTY_TOT) > 75 or sum(E_OQTY_TOT * E_PRICE)>3000) x
        ...
        INSERT  INTO #Customers
                ...
                FROM    dbo.TK_TRANS_ITEM_EVENT trans WITH ( NOLOCK )					
						inner join #hivalcust hc on trans.customer = hc.Customer 	
...
------------------------------------------------------------------
        ...
        INTO    #temp
        FROM    dbo.TK_TRANS_ITEM_EVENT trans WITH ( NOLOCK )
                INNER JOIN #Customers ON ( trans.CUSTOMER = #Customers.Customer )
